...
```
// ------ 第1步,编写需求，使用GPT O3------
请以@product_manager_req.md 要求，先了解faq页面的代码实现和数据库，
请了解@faq/ ，@faq/ ， @0007_create_faq_tables.sql 和 @faq/ ， 
在 doc/req/目录下编写一份 faq_add_video_req.md 的需求文档，包含需求，数据库设计和ux设计。

// ------ 第2步,分析如何实现，使用GPT O3------
请了解@faq/ ，@faq/ ，@0007_create_faq_tables.sql ，@0001_test_insert_faq_test_data.sql 等相关项目实现，分析如何实现这个功能。。

// ------ 第3步,设计数据库，使用claude 4 sonnet------
请按以上分析, 修改：

1. 浏览量防刷——不需要；
2.CSV 导入接口雏形	——不需要；

先查询现有数据库的情况，然后实现数据库相关开发和测试（测试都使用命令行），只需要完成数据库部分，列出todo后执行

// ------ 第4步,代码实现，使用claude 4 sonnet------
请根据以上分析和数据库设计，不需要执行pnpm run check。
请以@product_manager_req.md 要求，请先规划todo，然后再执行。
